...
const resendCodeService = async ({ email }: IResendCode): Promise<{ statusCode: number; message: string }> => {
  ...
  if (!foundUser) return { statusCode: 400, message: "User not found." };

  await Users.updateOne({ email }, { verificationCode });
  await Users.sendEmail(email, verificationCode);
...
const verificationService = async ({ email, code }: IVerification): Promise<{ statusCode: number; message: string }> => {
  ...
  if (foundUser.isVerified) return { statusCode: 400, message: "Account already verified." };
  if (foundUser?.verificationCode?.code !== code) return { statusCode: 400, message: "Invalid code" };
  ...
  await Users.updateOne({ email, isVerified: false }, { verificationCode: null, isVerified: true, isActivated: true });
...
// const logoutService = async (request: Request, response: Response) => request.logout();

export { registerService, verificationService, resendCodeService, resetPasswordService };
